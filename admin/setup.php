...
if(($_SERVER['REQUEST_METHOD']??'GET')==='POST'){
  requireAccountCsrf();$key=(string)($_POST['install_key']??'');
  if(!hash_equals((string)($config['install_key']??''),$key))$error='The installation key is incorrect.';
  else try{
...
<section class="card" style="max-width:600px;margin-inline:auto"><h1>One-time administrator setup</h1><p class="muted">Enter the installation key already stored in your Wishlist configuration. The signed-in account will become the first super administrator.</p><?php if($error):?><p class="error"><?=h($error)?></p><?php endif?><form method="post"><input type="hidden" name="csrf_token" value="<?=h(accountCsrf())?>"><label class="field">Installation key<input type="password" name="install_key" required></label><button>Install account system</button></form></section>
