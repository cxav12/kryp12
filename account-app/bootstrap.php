...
function renderAccountHeader(string $title,string $bodyClass=''): void { $u=accountUser();echo '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>'.h($title).' · KRYP12</title><link rel="stylesheet" href="/assets/account.css?v=2"></head><body class="'.h($bodyClass).'"><header class="account-top"><a href="/">KRYP12</a><nav>';if($u){echo '<a href="/account/">Account</a>';if(($u['role']??'')==='super_admin')echo '<a href="/admin/">Admin</a>';echo '<form method="post" action="/login/logout.php"><input type="hidden" name="csrf_token" value="'.h(accountCsrf()).'"><button>Sign out</button></form>';}else echo '<a href="/login/">Log in</a>';echo '</nav></header><main class="account-shell">'; }
